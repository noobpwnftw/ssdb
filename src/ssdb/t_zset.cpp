...
int64_t SSDBImpl::zsize(const Bytes &name){
	...
	s = ldb->Get(TERARKDB_NAMESPACE::ReadOptions(), size_key, &val);
	if(s.IsNotFound()){
		return 0;
	}else if(!s.ok()){
		return -1;
...
int SSDBImpl::zget(const Bytes &name, const Bytes &key, std::string *score){
	...
	TERARKDB_NAMESPACE::Status s = ldb->Get(TERARKDB_NAMESPACE::ReadOptions(), buf, score);
	if(s.IsNotFound()){
		return 0;
	}
	if(!s.ok()){
...
int64_t SSDBImpl::zfix(const Bytes &name){
	...
	while(it->next()){
		...
		std::string buf = encode_zset_key(name, key);
		std::string score2;
		s = ldb->Get(TERARKDB_NAMESPACE::ReadOptions(), buf, &score2);
		...
		if(s.IsNotFound() || score != score2){
			...
			s = ldb->Put(TERARKDB_NAMESPACE::WriteOptions(), buf, score);
			if(!s.ok()){
	...
	if(old_size != size){
		...
		std::string size_key = encode_zsize_key(name);
		if(size == 0){
			s = ldb->Delete(TERARKDB_NAMESPACE::WriteOptions(), size_key);
		}else{
			s = ldb->Put(TERARKDB_NAMESPACE::WriteOptions(), size_key, TERARKDB_NAMESPACE::Slice((char *)&size, sizeof(int64_t)));
		}
	}
	...
	while(it->next()){
		...
		if(s.IsNotFound()){
			...
			s = ldb->Put(TERARKDB_NAMESPACE::WriteOptions(), buf, "");
			if(!s.ok()){
				log_error("db error! %s", s.ToString().c_str());
				size = -1;
				break;
