...
static int qget_by_seq(TERARKDB_NAMESPACE::DB* db, const Bytes &name, uint64_t seq, std::string *val){
	std::string key = encode_qitem_key(name, seq);
	TERARKDB_NAMESPACE::Status s;

	s = db->Get(TERARKDB_NAMESPACE::ReadOptions(), key, val);
	if(s.IsNotFound()){
		return 0;
	}else if(!s.ok()){
		log_error("Get() error!");
...
static int qget_uint64(TERARKDB_NAMESPACE::DB* db, const Bytes &name, uint64_t seq, uint64_t *ret){
	std::string val;
	*ret = 0;
...
int64_t SSDBImpl::qsize(const Bytes &name){
	...
	TERARKDB_NAMESPACE::Status s;
	s = ldb->Get(TERARKDB_NAMESPACE::ReadOptions(), key, &val);
	if(s.IsNotFound()){
		return 0;
	}else if(!s.ok()){
		log_error("Get() error!");
...
int SSDBImpl::qfix(const Bytes &name){
	...
	if(count == 0){
		...
		this->binlogs->Put(encode_qsize_key(name), TERARKDB_NAMESPACE::Slice((char *)&count, sizeof(count)));
