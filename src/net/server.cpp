...
tbb::queuing_rw_mutex g_proc_mutex;
...
NetworkServer::NetworkServer(const Config &conf) {
	...
	memset(handoff_efd, 0, sizeof(handoff_efd));
	serve_max = 0;
	serve_accept = 0;
...
void *NetworkServer::serve(void *arg){
	...
	std::unordered_map<Command*, uint64_t> cmd_delta;
	std::unordered_map<int, Link*> conns;

	NetworkServer *net = (NetworkServer*)arg;
	int serve_idx = __sync_fetch_and_add(&net->serve_max, 1);
	int write_efd = eventfd(0, EFD_NONBLOCK);
	int handoff_efd = eventfd(0, EFD_NONBLOCK);
	net->handoff_efd[serve_idx] = handoff_efd;
	...
	fdes->set(write_efd, FDEVENT_IN, 0, NULL);
	fdes->set(handoff_efd, FDEVENT_IN, 0, NULL);
	...
	while(!quit){
		...
		for(int i=0; i<(int)events->size(); i++){
			...
			if(fde->fd == write_efd){
				uint64_t v; while (::read(write_efd, &v, sizeof(v)) > 0) {}
				ProcJob job;
				bool handoff = false;
				while(wq->pop(&job)){
					...
					if(!link->error()){
						if(!handoff && link->output->empty()){
							handoff = true;
							int min_idx = serve_idx;
							int *pcount = (link->remote_port != 0) ? net->link_count : net->link_count_unix;
							int cnt = pcount[serve_idx % SERVE_THREADS] - 1;
							for(int j = 1; j < net->serve_max; j++){
								int idx = (serve_idx + j) % net->serve_max;
								if(net->handoff_efd[idx] > 0 && pcount[idx % SERVE_THREADS] < cnt){
									min_idx = idx;
									break;
								}
							}
							if(min_idx != serve_idx){
								...
								// proc_result clears OUT when link->output->empty(), and IN was cleared before
								// handing the request to a worker, so this fd is already removed from this poller.
								//fdes->del(link->fd());
								conns.erase(link->fd());
								net->handoff_queue[min_idx].push(link);
								uint64_t one = 1; ::write(net->handoff_efd[min_idx], &one, sizeof(one));
								continue;
							}
						}
			...
			}else if(fde->fd == handoff_efd){
				uint64_t v; while (::read(handoff_efd, &v, sizeof(v)) > 0) {}
				Link* link;
				while(net->handoff_queue[serve_idx].pop(&link)){
					conns.emplace(link->fd(), link);
					if(link->remote_port != 0)
						__sync_add_and_fetch(&net->link_count[serve_idx % SERVE_THREADS], 1);
					else
						__sync_add_and_fetch(&net->link_count_unix[serve_idx % SERVE_THREADS], 1);
					fdes->set(link->fd(), FDEVENT_IN, 1, link);
					if(!link->input->empty())
						ready_list.push_back(link);
				}
		...
		bool backlogged = ready_list.size() > 2;
		cmd_delta.clear();
		for(it = ready_list.begin(); it != ready_list.end(); it ++){
			...
			do {
				...
				job.wq = wq;
				job.efd = write_efd;
				int result = net->proc(&job, link, backlogged);
				if(job.cmd){
					cmd_delta[job.cmd]++;
				}
			...
			} while(!link->error() && !link->input->empty());
		} // end foreach ready link
		for(auto& elem : cmd_delta){
			elem.first->calls.fetch_add(elem.second, std::memory_order_relaxed);
		}
	}
	net->workers->stop();
	{
		Link* link;
		std::unordered_map<int, Link*>::iterator iter = conns.begin();
...
void NetworkServer::proc_result(Fdevents *fdes, ProcJob *job, Link* link, ready_list_t *ready_list_2){
...
int NetworkServer::proc(ProcJob *job, Link* link, bool backlogged){
	job->serv = this;
	job->result = PROC_OK;
	...
	do{
		// AUTH
		const Bytes &cmd_name = (*req)[0];

		// AUTH
		if(need_auth && link->auth == false && cmd_name != "auth"){
			job->resp.push_back("noauth");
			job->resp.push_back("authentication required.");
			break;
		}

		job->cmd = proc_map.get_proc(cmd_name);
		if(!job->cmd){
			job->resp.push_back("client_error");
			job->resp.push_back("Unknown Command: " + cmd_name.String());
			break;
		}
		...
		if(readonly && (job->cmd->flags & Command::FLAG_WRITE)){
			job->resp.push_back("client_error");
			job->resp.push_back("Forbidden Command: " + cmd_name.String());
			break;
		}

		if(job->cmd->flags & Command::FLAG_THREAD
			|| ((job->cmd->flags & Command::FLAG_THREAD_SOFT) && backlogged)){
			workers->push(*job);
			return PROC_THREAD;
		}
