...
private:
	struct alignas(64) Slot {
		...
		T item;
		char pad[64 - (sizeof(std::atomic<uint64_t>) + sizeof(T)) % 64];
	};
	alignas(64) Slot ring_[CAP];

	alignas(64) std::atomic<uint64_t> tail_{ 0 };
	char _pad1[64 - sizeof(std::atomic<uint64_t>) % 64];
	alignas(64) std::atomic<uint64_t> head_{ 0 };
	char _pad2[64 - sizeof(std::atomic<uint64_t>) % 64];
public:
	Queue();
